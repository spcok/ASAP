...
const SystemHealth: React.FC = () => {
  ...
  const runPipelinePing = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('daily_logs_v2').select('id').eq('id', testId).single();
      ...
      await supabase.from('daily_logs_v2').delete().eq('id', testId);
