...
export async function prune14DayCache() {
  ...
  try {
    await Promise.all([
      db.daily_logs_v2.where('log_date').below(isoDate).delete(),
...
export async function forceHydrateFromCloud() {
  const tables = [
    'animals', 'daily_logs_v2', 'medical_logs', 'tasks', 'users', 
    'role_permissions', 'organisations', 'contacts', 'zla_documents',
    'incidents', 'daily_rounds', 'operational_lists', 'maintenance_logs'
  ];
...
export async function reconcileMissedEvents() {
  const tables = [
    'animals', 'archived_animals', 'daily_logs_v2', 'medical_logs', 'tasks', 
    'incidents', 'internal_movements', 'external_transfers', 'daily_rounds', 
    'operational_lists', 'mar_charts', 'quarantine_records', 'shifts', 
    'timesheets', 'holidays', 'maintenance_logs', 
    'organisations', 'contacts', 'zla_documents', 'users', 'role_permissions'
  ];
